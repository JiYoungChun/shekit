function Ball() {
  ...
  this.display = function() {
    ...
    noStroke();
    fill(random(100, 255), random(100, 255), random(150, 255), 150);
    //if (f)
//image(dot,this.x-this.diameter,this.y-this.diameter,this.diameter+10, this.diameter+10);
   ellipse(this.x, this.y, this.diameter, this.diameter);

    textSize(8);
    //text(this.venue, this.x + 8, this.y + 3);

    if (touchIsDown && touchX > this.x - this.diameter && touchX < this.x + this.diameter && touchY > this.y - this.diameter && touchY < this.y + this.diameter) {
  ...
  if (this.distance < 50) {
    ...
    if (this.f > -1) {
      ...
      image(foodIcon, this.x - 6, this.y - 6, 12, 12);
    }

    if (this.dessert > -1) {

      image(dessertIcon, this.x - 6, this.y - 6, 12, 12);
    }
    if (this.bar > -1) {
      //fill(0,255,255);
      //ellipse(this.x,this.y,10,10);
      image(barIcon, this.x - 6, this.y - 6, 12, 12);
    }
